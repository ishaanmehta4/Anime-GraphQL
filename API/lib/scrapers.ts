...
export let getCharacterData = async function (characterName: string):Promise<[any, any]> {
  try {
    ...
    characterData.name = dom
      .querySelector('#main1 > h1 > a.fgw')
      .textContent.replace(/\r?\n|\r/g, ' ')
      .trim();
    characterData.bio = dom
      .querySelector('#characterzone > table > tbody > tr > td:nth-child(2) > p')
      .textContent.replace(/\r?\n|\r/g, ' ')
      .trim();
    ...
    dom.querySelectorAll('#sidephoto > table:nth-child(4) > tbody > tr').forEach((row: HTMLElement) => {
    ...
    characterData.voicedby = characterData.voicedby.split(',').filter((name: string) => name.trim() != '');
    characterData.tags = characterData.tags.split(',').map((tag: string) => tag.trim());
...
export let getAnimeData = async function (animeName: string):Promise<[any, any]> {
  try {
    ...
    animeData.name = dom
      .querySelector('#main1 > h1')
      .textContent.replace(/\r?\n|\r/g, ' ')
      .trim()
      .split(' | ')[0];
    animeData.plot = dom
      .querySelector('#mainframe2 > #besttable > table > tbody > tr > td')
      .textContent.replace(/\r?\n|\r/g, ' ')
      .trim();
    ...
    dom.querySelectorAll('#tile1 > ul > li').forEach((card: HTMLElement) => {
      let tempDom = new jsdom.JSDOM(card.innerHTML);
      tempDom = tempDom.window.document;

      if (tempDom.querySelector('a')) animeData.characterNames.push(tempDom.querySelector('a').textContent.trim());
    });
